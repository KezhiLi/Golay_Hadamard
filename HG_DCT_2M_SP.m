...
for jj = 1:knum,
    ...
    for i=1:trial_num
        ...
        % observation
        y1 = Gauss_op*idct(eye(N))*x;
        y2 = Zad_op*idct(eye(N))*x;
        y3 = WHT_op*idct(eye(N))*x; 
        y4 = CDM_op*idct(eye(N))*x;
   ...
   %     alp1 = omp(x, Gauss_op, y1, k, sigma);
      ...
      alp1 = CSRec_SP(k, Gauss_op*idct(eye(N)), y1);
  ...
  %      [alp1 iter_num1] = SAMP(y1, Gauss_op*dct(N), step_size, sigma);
   ...
   %     alp2 = omp(x, Zad_op, y2, k, sigma);
      ...
      alp2 = CSRec_SP(k, Zad_op*idct(eye(N)), y2);
  ...
  %      [alp2 iter_num1] = SAMP(y2, Zad_op*dct(N), step_size, sigma);
   ...
   %     alp3 = omp(x, WHT_op, y3, k, sigma);
      ...
      alp3 = CSRec_SP(k, WHT_op*idct(eye(N)), y3);
   ...
   %     alp4 = omp(x, CDM_op, y4, k, sigma);
      ...
      alp4 = CSRec_SP(k, CDM_op*idct(eye(N)), y4);
