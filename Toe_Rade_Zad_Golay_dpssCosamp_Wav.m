...
%         y0 = Gauss_opRD*(x0((jj-1)*const+1:jj*const));
%         y1 = Gauss_op*(x0((jj-1)*const+1:jj*const));
%         y2 = Zad_op*(x0((jj-1)*const+1:jj*const));
%         y3 = WHT_op*(x0((jj-1)*const+1:jj*const));
%         y4 = CDM_op*(x0((jj-1)*const+1:jj*const));
        
        % observation
        y0 = Gauss_opRD*ifft(F_0((jj-1)*const+1:jj*const));
        y1 = Gauss_op*ifft(F_0((jj-1)*const+1:jj*const));
        y2 = Zad_op*ifft(F_0((jj-1)*const+1:jj*const));
        y3 = WHT_op*ifft(F_0((jj-1)*const+1:jj*const));
        y4 = CDM_op*ifft(F_0((jj-1)*const+1:jj*const));
   ...
   [alp0 ] = dpssCosamp(y0,senseOpts,K,B,cosampOpts); % Recover via Cosamp
  ...
  %      [alp1 iter_num1] = SAMP(y1, Gauss_op, step_size, sigma);
     ...
     [alp1 ] = dpssCosamp(y,senseOpts,K,B,cosampOpts); % Recover via Cosamp
   ...
   %     alp2 = omp(x, Zad_op, y2, k, sigma);
    ...
    [alp2 ] = dpssCosamp(y,senseOpts,K,B,cosampOpts); % Recover via Cosamp
   ...
   %     alp3 = omp(x, WHT_op, y3, k, sigma);
    ...
    [alp3 ] = dpssCosamp(y,senseOpts,K,B,cosampOpts); % Recover via Cosamp
   ...
   %     alp4 = omp(x, CDM_op, y4, k, sigma);
    ...
    [alp4 ] = dpssCosamp(y,senseOpts,K,B,cosampOpts); % Recover via Cosamp
