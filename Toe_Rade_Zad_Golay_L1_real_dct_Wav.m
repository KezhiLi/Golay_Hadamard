...
%         y4 = CDM_op*(x0((jj-1)*const+1:jj*const));
        ...
        % observation
        ifft_F0_blk = idct(F_0((jj-1)*const+1:jj*const));  %%%idct
        %real_ifft_F0_blk = [real(ifft_F0_blk); imag(ifft_F0_blk)];
        y0 = Gauss_opRD*ifft_F0_blk;
        y1 = Gauss_op*ifft_F0_blk;
        y2 = Zad_op*ifft_F0_blk;
        y3 = WHT_op*ifft_F0_blk;
        y4 = CDM_op*ifft_F0_blk;
...
%              F_CDM((jj-1)*const+1:jj*const) = alp4;
            F_GaussRD((jj-1)*const+1:jj*const) = dct(alp0);
            F_Gauss((jj-1)*const+1:jj*const) = dct(alp1);
            F_Zad((jj-1)*const+1:jj*const) = dct(alp2);
            F_WHT((jj-1)*const+1:jj*const) = dct(alp3);
             F_CDM((jj-1)*const+1:jj*const) = dct(alp4);
