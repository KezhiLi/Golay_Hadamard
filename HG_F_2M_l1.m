...
aaa=exp(i*2*rand(1,(thesize/2-1))*pi );  % random phase for random convolution
...
matr15=matr5(rp22(1:MM),:);

%matr5=[];
%matr1=matr(1:1:1*M,:);
matr15=matr15*sqrt(N/MM);
...
Hada1=hadamard(N);
GolayDia=diag(generate_golay(log2(N)));

% HG=Hada1*GolayDia;
% HGrp=randperm(N);
% HG_op=HG(HGrp(1:M),:);

HGF=Hada1*GolayDia;%*dftmtx(N);
HGFrp=randperm(N);
HGF_op=HGF(HGFrp(1:MM),:);
CDM_op=HGF_op;
%CDM_op=[real(CDM_op);imag(CDM_op)];
...
for jj = 1:knum,
    ...
    for i=1:trial_num
        ...
        % observation
        y1 = Gauss_op*conj(dftmtx(N))*x;
        y2 = Zad_op*conj(dftmtx(N))*x;
        y3 = WHT_op*conj(dftmtx(N))*x; 
        y4 = CDM_op*conj(dftmtx(N))*x;
   ...
   %     alp1 = omp(x, Gauss_op, y1, k, sigma);
      ...
      %alp1 = CSRec_SP(k, Gauss_op*conj(dftmtx(N)), y1);
  ...
  %      [alp2 iter_num1] = SAMP(y2, Zad_op*dct(N), step_size, sigma);
   ...
   %     [alp4 iter_num4] = SAMP(y4, CDM_op*dct(N), step_size, sigma);
        ...
        [alp4,Out4] = fpc_AS(N,CDM_op*conj(dftmtx(N)),y4,mu,[]);
...
grid
%success_Gauss
legend('Random Gaussian', 'Random Gaussian Toeplitz','Random convolution', 'Golay-paired Hadamard matrix');
